...
namespace LoggerTest
{
    ...
    class Program
    {
        ...
        /// <summary>
        /// This is the method that should be pointed at your logger....
        /// </summary>
        private static void Log(string text, int level, Exception ex = null)
        {
            SampleLogger.Logger logger = new SampleLogger.Logger();
            logger.Log(text, (SampleLogger.LogLevelSampleEnum)level, ex);
        }
            ...
            for (int i = 1; i <= NUMBER_OF_THREADS_SENDING_MESSAGES; i++)
            {
                ...
                Thread newThread = new Thread(x =>
                {
                    ...
                    for (int messageNumber = 1; messageNumber <= NUMBER_OF_MESSAGES_PER_THREAD; messageNumber++)
                    {
                        ...
                        Log(string.Format(errorMessage, threadNumber, messageNumber, logLevel), logLevel, (messageNumber % HOW_OFTEN_AN_EXCEPTION == 0) ? exception : null);
                    }
                });
